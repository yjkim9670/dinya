...
GitHub Pages로 배포되는 차콜 그레이 테마의 트레이딩 콘솔입니다. 삼성전자·SK하이닉스와 TIGER S&P500·TIGER 나스닥100을 추적하며, 기술적 지표 기반의 매매 추천 점수와 자동 모의투자 잔고를 함께 제공합니다. 최근 30거래일을 기본으로 최대 9개월(180거래일)까지 일별 가격 흐름을 살펴볼 수 있으며, 차트에는 매수/매도가 적절했던 시점이 시각적으로 표시됩니다.

## 주요 기능

- **차콜 그레이 다크 테마**: 눈의 피로를 줄이는 다크 모드 UI와 밝은 액센트 컬러(스카이 블루)를 적용했습니다. 폰트는 Source Sans Pro와 Noto Sans KR 조합을 사용합니다.
- **반응형 메뉴 바**: 상단에 고정된 네비게이션 바로 대시보드, 포트폴리오, 분석, 설정 메뉴에 빠르게 접근할 수 있습니다. 모바일에서는 햄버거 메뉴로 전환됩니다.
...
- **6개월 이상 차트**: 슬라이더로 최근 30~180거래일(약 9개월) 범위를 즉시 조절하며, 20분 주기로 데이터가 새로 고쳐집니다.
- **매수/매도 시그널 표시**: 차트에 과거 매수(초록 삼각형)와 매도(빨간 역삼각형) 시점이 시각적으로 표시되어 전략의 효과를 직관적으로 확인할 수 있습니다.
- **기술적 지표·시그널 태그**: RSI·스토캐스틱·MACD 등 핵심 지표와 해석 태그를 카드 내 섹션으로 정리해 한눈에 파악할 수 있습니다.
...
1. `scripts/fetch_market_data.py`가 yfinance에서 최근 9개월 일별 데이터를 가져와 180거래일 분량의 차트 히스토리를 만듭니다.
2. SMA(5·20), RSI(14), 스토캐스틱(14,3,3), MACD(12,26,9)을 계산하고, 시그널 가중치로 추천 점수(0~100)를 산출합니다.
3. **전체 히스토리를 순회하며 각 시점의 매수/매도 시그널을 기록**하여 차트에 표시할 수 있도록 준비합니다.
4. 추천 점수에 따라 종목별 모의투자 포트폴리오를 자동 갱신합니다. 80점 이상은 가능한 만큼 매수, 20점 이하는 전량 매도하며, 잔고는 `data/portfolio.json`에 저장됩니다.
5. `data/latest.json`에는 지표·시그널·추천 점수·모의투자 현황·매매 시그널이 함께 기록되고, `data/history/*.csv`에는 전체 히스토리가 누적됩니다.
6. `.github/workflows/update-data.yml`이 매 정시(`0 * * * *`)마다 실행되어 JSON과 포트폴리오 파일을 갱신하면, 프런트엔드는 20분 주기로 최신 데이터를 다시 불러옵니다.
...
모의투자가 어떻게 작동하는지 자세히 알고 싶다면 [MOCK_TRADING_GUIDE.md](MOCK_TRADING_GUIDE.md)를 참고하세요. 매매 규칙, 실행 흐름, 성과 측정 방법, 실제 예시까지 상세히 설명되어 있습니다.
...
- **디지털 자산 시세**: 업비트 KRW 마켓 심볼(`KRW-BTC`, `KRW-ETH`, `KRW-SOL`)을 사용하며, 프런트엔드 카드와 포트폴리오 계산도 원화 기준으로 동작합니다.
